...
public final class LaucherCobblemon {
    ...
    private static void createWindow() {
        ...
        header.add(title); header.add(Box.createVerticalStrut(6)); header.add(version);
        ...
        install.addActionListener(e -> {
            ...
            install.setEnabled(false); progressBar.setIndeterminate(true); status.setText("Preparando…");
            new SwingWorker<Void, String>() {
                @Override protected Void doInBackground() throws Exception {
                    SERVICE.prepare(selectedFolder[0], message -> SwingUtilities.invokeLater(() -> { status.setText(message); progressBar.setString(message); }));
                    return null;
                }
                @Override protected void done() {
                    install.setEnabled(true); progressBar.setIndeterminate(false);
                    try { get(); progressBar.setString("Preparado"); status.setText("Listo para jugar."); }
                    catch (Exception ex) { Throwable cause = ex.getCause() != null ? ex.getCause() : ex; progressBar.setString("Error"); status.setText("Error: " + cause.getMessage()); JOptionPane.showMessageDialog(frame, cause.getMessage(), "Error", JOptionPane.ERROR_MESSAGE); }
                }
            }.execute();
        });
        ...
        play.addActionListener(e -> {
            ...
            play.setEnabled(false); status.setText("Iniciando Minecraft…"); progressBar.setIndeterminate(true);
            new SwingWorker<Void, Void>() {
                @Override protected Void doInBackground() throws Exception {
                    SERVICE.launch(selectedFolder[0], (Integer) ram.getValue(), message -> SwingUtilities.invokeLater(() -> status.setText(message)));
                    return null;
                }
                @Override protected void done() {
                    play.setEnabled(true); progressBar.setIndeterminate(false);
                    try { get(); progressBar.setString("Minecraft iniciado"); status.setText("Minecraft se está ejecutando."); }
                    catch (Exception ex) { Throwable cause = ex.getCause() != null ? ex.getCause() : ex; progressBar.setString("Error"); status.setText("Error al iniciar"); JOptionPane.showMessageDialog(frame, cause.getMessage(), "Error al iniciar Minecraft", JOptionPane.ERROR_MESSAGE); }
                }
            }.execute();
        });
        ...
        actions.add(install, BorderLayout.CENTER); actions.add(play, BorderLayout.EAST);
        bottom.add(actions, BorderLayout.NORTH); bottom.add(status, BorderLayout.CENTER); bottom.add(progressBar, BorderLayout.SOUTH);
        root.add(header, BorderLayout.NORTH); root.add(settings, BorderLayout.CENTER); root.add(bottom, BorderLayout.SOUTH);
